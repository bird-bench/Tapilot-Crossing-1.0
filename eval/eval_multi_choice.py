...
# Function to parse the workplace.txt content and extract options and correct answer  
def parse_workplace_content(content):  
...
def eval_main(json_file_path, setting, model_use, action_name):
    ...
    for path, text_with_answer in data.items():  
        ...
        if answer == '':
            ...
            # path = path.replace("/path/to/old/data/", "/path/to/new/data/")
            # Construct the path to the workplace.txt file  
            workplace_file_path = os.path.join(path, 'workplace.txt')  

            # Check if the workplace.txt file exists  
            if os.path.exists(workplace_file_path):  
                # Load the content of the workplace.txt file  
                with open(workplace_file_path, 'r', encoding='utf-8') as f:  
                    workplace_content = f.read()  

                # Parse the content to get the options and correct answer  
                options, correct_answer = parse_workplace_content(workplace_content)  
                ...
                incorrect_count += 1
                stats_dict["ex"] = 0
                print(f'Could not find workplace.txt file at path "{path}".')  
